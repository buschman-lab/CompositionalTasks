...
function [slop,asymptoteDiff,params] = fitAndAdjustSigmoidLsq(x, y,PlotFlag,varargin)
    ...
    if PlotFlag; fit_curve = sigmoid_model(params, x);end
    ...
    if slope < 0
        ...
        if PlotFlag; fit_curve = descending_sigmoid_model(params, x);end
    ...
    if PlotFlag
        ...
        title(sprintf('Sigmoid Fit: %s Slop:%0.2f min:%0.2f max:%0.2f diff:%0.2f',txt,slop,params(1),params(2),asymptoteDiff) );
        pause
    end
