function PlotFreq_AmpPhase(data,varargin) 
%%% plots phase and amplitude of freq spectrum signal 
%%% inputs 
...
k=10; 
MeanPSD=movmean(s_all,k,3);
figure;set(gcf,'Units','Normalized','Position',[0 0 0.9 0.9])

%  
colormap(jet);%axisLim=[0 5e-5];
...
%     spectrogram(data(:,i),Window,noverlap,[],opts.Fs,'yaxis');
%     imagesc(MeanCutPSD(:,:,i));colorbar
   %  caxis(axisLim)
%     xticks(XTik);yticks(YTik);
%     xticklabels(arrayfun(@(x) num2str(Time(x),3),XTik,'Uniformoutput',0))
%     yticklabels(arrayfun(@(x) num2str(w(x),3),YTik,'Uniformoutput',0))
...
%     ylabel('PSD')
   % set(gca,'YDir','normal')
    ...
    title(sprintf('Ch %s Area %s Trial %s',num2str(AnalysisOpts.CurrentChNum),AnalysisOpts.CurrentChArea,num2str(i)));
...
% now make the movie 
fps=2;
MakeMovieFromFrames(mvFrame,fps,saveMovieStr)
