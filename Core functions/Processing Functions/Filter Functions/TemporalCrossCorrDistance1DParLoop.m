...
% uses 1D correlation
% works with PArloop
function [D,Shift,EuDist] = TemporalCrossCorrDistance1DParLoop(i,j,Motifs,SizeW)
ZJ=Motifs(j,:);
...
for zj=1:size(ZJ,1)
    ...
    NewMotif=circshift(ZJ_pad,Shift(zj),2); % shift to get anew motif
    % calculate Eauclidean distance between template and new motif
    EuDist(zj)=EauclidDist(ZI_pad,NewMotif);
    ...
    if PlotDetails
        ...
        subplot(144)
        helperCWTTimeFreqPlot(NewMotif,1:size(NewMotif,2),AnalysisData.cwt_f,'justplot1',['dist ' num2str(EuDist,2) ],'Time','Frequency(Hz)',0)
        pause
    end
end
...
function Dist=EauclidDist(X1,X2)
