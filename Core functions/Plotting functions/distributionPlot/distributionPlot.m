        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    if dx < 1e-6 %put a catch to stop dx from being tiny which will lead to a huge x variable
                        dx = 1e-6;
                    end
                    x = min(x)-dx:dx/1000:max(x)+dx;                
%                   x = min(x)-dx:dx/3:max(x)+dx
                    [xHist,yHist] = ksdensity(currentData,x,'kernel','epanechnikov','width',dx/(1.5*opt.divFactor));
                    end
                else
                    ...
                    if opt.divFactor ~= 1
                        [xHist,yHist] = ksdensity(currentData,'kernel','epanechnikov','width',u/opt.divFactor);
                    end
                end
...
%             hh(iData)= patch('Vertices',vertices,'Faces',faces,'Parent',ah,colorOpt{:},'LineWidth',0.5);
        case 'flipped'
            hh(iData)= patch('Vertices',vertices(:,[2,1]),'Faces',faces,'Parent',ah,colorOpt{:},'EdgeColor','none');
%             hh(iData)= patch('Vertices',vertices(:,[2,1]),'Faces',faces,'Parent',ah,colorOpt{:},'LineWidth',0.5);
    end
