classdef ClusterFuncsDella   
    ...
    properties
       ...
       % script_path='Z:\Users\Sina\Rule Representation Project\Server Analysis forlders\Rule Representation Cluster Analysis Functions\'
        % function_path='/jukebox/buschman/Projects/Rule_Representation/ElecPhys_Analysis/Data Analysis Pipelines/Oscillations Breakdown ToolBox/Pipeline/'
        ...
        CompResource='della'; % what computational rescource we are using spock or della
    end
...
%             end % if it already exists delete it 
         ...
         %   fprintf(fid,'\n#SBATCH --mem-per-cpu=%iG',obj.Memory);            % add memory size 
            ...
            try %make sure to close the fid even if crash
               ...
               fprintf(fid, ['xvfb-run -d matlab -nosplash -nodisplay -nodesktop -r ',...
                   sprintf('"addpath(''/jukebox/buschman/Projects/Rule_Representation/ElecPhys_Analysis/Core functions/Cluster functions/'');CF=ClusterFuncs;'),... % add cluster functions
        ...
        function s_conn=Connect2Cluster(obj)
            % Open ssh connection
            username = 'tafazoli';%input(' Spock Username: ', 's');
            if isempty(obj.password)
                password = input('Spock Password: ', 's');
            else
        ...
        function Classifier_TaskSpockTime=GetClassifierSpockTime(~) % gets requested time for running classifer 
            ...
            elseif AnalysisOpts.DividSpockClassifier==3
                ...
                if ~AnalysisOpts.CalShuffleClassifier
                    FullFoldRunTime=floor(AnalysisOpts.CurrentClassifierOpts.Nrep*nCV*30/60)+20; % this 10 folds 40 sec for fold 60 mins to start
                else
                   ...
                   if AnalysisOpts.CalShuffTrlOrderClassifier
                       FullFoldRunTime=floor(AnalysisOpts.CurrentClassifierOpts.NrepShuf*nCV*50/60)+20; % this 10 folds 20 sec for fold 60 mins to start
                   else
                       FullFoldRunTime=floor(AnalysisOpts.CurrentClassifierOpts.NrepShuf*nCV*30/60)+20; % this 10 folds 20 sec for fold 60 mins to start
                   end
                end
            ...
            if AnalysisOpts.ProcessingStep==4
                if AnalysisOpts.RunCrossTemporalClassifer
                    Classifier_TaskSpockTime=800*ones(1,length(Classifier_TaskSpockTime));
                else
                    Classifier_TaskSpockTime=30*ones(1,length(Classifier_TaskSpockTime));
                end               
            end
