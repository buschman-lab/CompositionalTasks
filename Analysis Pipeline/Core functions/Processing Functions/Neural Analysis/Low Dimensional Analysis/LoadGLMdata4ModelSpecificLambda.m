function [GLMWeightsAll,GLMCPDAll,Mdl,Time,TimeIndGLM,WantedFieldName]=LoadGLMdata4ModelSpecificLambda(TimeRef,Area,MdlName,Lambda)
...
WantedFieldNameCPD=FieldNamesCPD(~contains(FieldNamesCPD,'Time')  & ...
...
for Factor=WantedFieldNameCPD
    IndFactor=find(strcmp(FieldNames,Factor{1}));    % get CPD values
    GLMCPDAll.(Factor{1})=cell2mat(arrayfun(@(x) Mdl(x).(GLMModelTxt).CPD.(Factor{1})(Lambda,TimeIndGLM)',NNeu,'UniformOutput',0));
end
end