function [logp,pout]=myBinomTestLogScale(s,n,p,Sided)

pout=arrayfun(@(x) 1-binocdf(s(x),n(x),p),1:length(n));

% Exact binomial test in log space returns log10(p-value) to avoid underflow

% Probability of observed outcome
logp_obs = log(binopdf(s,n,p));

% All possible outcomes
k = 0:n;
logprobs = log(binopdf(k,n,p));
% One-sided test (upper tail)
mask = k >= s;
% log-sum-exp trick
logp = logsumexp(logprobs(mask));
% Return in base-10 for reporting
logp = logp / log(10);
end
