classdef ClusterFuncsTemp
    ...
    properties
       ...
       %  clust_status_path_PC='/Volumes/buschman/Projects/Rule_Representation/ElecPhys_Analysis/ClusterStatus/';
        ...
        function_path='/jukebox/buschman/Projects/Rule_Representation/ElecPhys_Analysis/Rule Representation Project/Submission Code/Final Code for Nature Paper 2025 Tafazoli et al/Analysis Pipeline/Code/Pipeline/Primary Analysis/'
...
%             end % if it already exists delete it 
         ...
         %   fprintf(fid,'\n#SBATCH --mem-per-cpu=%iG',obj.Memory);            % add memory size 
            ...
            try %make sure to close the fid even if crash
               ...
               fprintf(fid, ['xvfb-run -d matlab -nosplash -nodisplay -nodesktop -r ',...
                   sprintf('"addpath(''/jukebox/buschman/Projects/Rule_Representation/ElecPhys_Analysis/Rule Representation Project/Submission Code/Final Code for Nature Paper 2025 Tafazoli et al/Analysis Pipeline/Core functions/Cluster functions/'');CF=ClusterFuncsTemp;'),... % add cluster functions
        ...
        function [response,job_id,script_name]=RunFunctiononCluster(obj,job_name,func_name,input_val,input_type,varargin)
            ...
            if ~strcmp(obj.CompResource,'spock')
                ...
                ssh2_close(s_conn);
            else % then the connection doesn't work anymore and should be run locally on scotty through the sbatch command
                fprintf(2,['\n ssh username@scotty.princeton.edu \n cd jukebox>buschman>Users>Sina Rule Representation Project>Server Analysis Forlders>Cluster Analsis General Functions \n' ...
                    'Run via Command Window: Script File Name: %s\n'],script_name)
                job_id=0;
                response='';
                pause
            end
        ...
        function s_conn=Connect2Cluster(obj)
            ...
            % change the name of spock to scotty as of 09-12-2025
            if strcmp(obj.CompResource,'spock')
         %      s_conn = ssh2_config('scotty.pni.princeton.edu',username,password);
                s_conn = ssh2_config_publickey('scotty.princeton.edu', ...
                               'tafazoli', ...
                               'C:/Users/mouse6/.ssh/id_rsa', ...
                               '',22); 
              %  s_conn = ssh2_command(s_conn, 'echo hello');
...
%             pause(PauseTimeOrder(SLURM_ARRAY_TASK_ID)/20+randsample([0.1:0.1:3],1)); % pause so that eveything is not written at the same time 
        ...
        function Classifier_TaskSpockTime=GetClassifierSpockTime(~) % gets requested time for running classifer 
            ...
            if contains(AnalysisOpts.CurrentClassifierOpts.Name,'Learning');
                TrialRangeSet=AnalysisOpts.CurrentClassifierOpts.TestTrlNumRange{1}(2:4);
                TrialRange=TrialRangeSet(2):TrialRangeSet(3):abs(TrialRangeSet(1));
                nTrlRng=length(TrialRange);      
            else
                nTrlRng=1;
            end
            if nTrlRng==0;nTrlRng=1;end
            ...
            elseif AnalysisOpts.DividSpockClassifier==3
                ...
                if AnalysisOpts.CalShuffTrlOrderClassifier
                    if AnalysisOpts.Classifier_TrlShuff_GenClassifierSpecs % if we are generating the classifier file 
                        FullFoldRunTime=700;
                    else 
                        if AnalysisOpts.CalShuffTrlOrderClassifier_RunAllTrlRng==1 % if we are running all of the trial ranges 
                            FullFoldRunTime=800;
                        else
                            FullFoldRunTime=500;
                        end
                    end
                elseif ~AnalysisOpts.CalShuffleClassifier & ~AnalysisOpts.NeuResampling
                     if AnalysisOpts.AreaNum==1 % for PFC adjust timing
                         FullFoldRunTime=floor(AnalysisOpts.CurrentClassifierOpts.Nrep*nCV*22/60)+20; % this 10 folds 12 sec for fold 60 mins to start
                     else
                         FullFoldRunTime=floor(AnalysisOpts.CurrentClassifierOpts.Nrep*nCV*8/60)+20; % this 10 folds 6 sec for fold 60 mins to start
                     end
                elseif ~AnalysisOpts.CalShuffleClassifier & AnalysisOpts.NeuResampling % if we are doing neuron resampling 
                    if AnalysisOpts.NeuResamplingSubPopSize>150 % if the number of neurons are high then request max time
                        FullFoldRunTime=2450;
                    else
                        FullFoldRunTime=2400;
                    end
                else
                   ...
                   if AnalysisOpts.CurrentClassifierOpts.SkipTimeBinClassifierFitting4Shuff
                           FullFoldRunTime=floor(AnalysisOpts.CurrentClassifierOpts.NrepShuf*nTrlRng*30*NConds/60); % this 10 folds 20 sec for fold 60 mins to start                       
                  
                   elseif AnalysisOpts.UseRep4Cluster & contains(AnalysisOpts.CurrentClassifierOpts.Name,'Learning3D') % we are running each shuffle repetition seperately
                       FullFoldRunTime=floor(AnalysisOpts.CurrentClassifierOpts.NrepShuf*70*16/60); % this 10 folds 20 sec for fold 60 mins to start                       
        ...
        function TDR_SpockTime=GetTDRSpockTime(~)% gets requested time for running GLM analysis
            global AnalysisOpts
            
            switch AnalysisOpts.ProcessingStep
                case 17 % run TDR and plot
                    TDR_SpockTime=AnalysisOpts.TDRSpockTime;
