...
function [slop,asymptoteDiff,params,bounded_asymptoteDiff,Norm_bounded_asymptoteDiff,Norm_bounded_asymptoteDiffMin] = fitAndAdjustSigmoidLsq(x, y,PlotFlag,varargin)
    % Fit a sigmoid function to the data (x, y) and adjust for ascending or descending trend
    % bounded_asymptoteDiff is bounded difference of min and max of y in the bounds of x
    ...
    options = optimset('Display', 'off','MaxIter', 1000,'TolFun', 1e-6,'Algorithm','levenberg-marquardt');
    ...
    slope = params(2) - params(1);  % Slope of the sigmoid curve
    slopesign=double(sign(slope));
    if slope < 0
    ...
    slop=params(4);
    asymptoteDiff=params(2)-params(1);
    bounded_asymptoteDiff=(max(fit_curve)-min(fit_curve))*slopesign;
    Norm_bounded_asymptoteDiff=(max(fit_curve)-min(fit_curve))*slopesign/(max(fit_curve)+min(fit_curve));
   
    Norm_bounded_asymptoteDiffMin=(max(fit_curve)-min(fit_curve))*slopesign/(min(fit_curve));
    ...
    if PlotFlag
        ...
        ylabel('Y-axis');
        legend('show');
        if ~isempty(varargin);txt=varargin{1};else;txt='';end
        title(sprintf('Sigmoid Fit: %s Slop:%0.2f min:%0.2f max:%0.2f diff:%0.2f bndDiff:%0.2f Nrm_bndDiff:%0.2f',txt,slop,params(1),params(2),asymptoteDiff,bounded_asymptoteDiff,Norm_bounded_asymptoteDiff) );
        pause
    end
