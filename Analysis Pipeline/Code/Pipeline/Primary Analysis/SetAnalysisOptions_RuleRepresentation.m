...
AnalysisOpts.NcoresSpock=[]; % what is the current analysis NcoresSpock
AnalysisOpts.NcoresSpockProcessingStep=[5*ones(1,16) 6]; % Number of cores for spock for specfic analysis
AnalysisOpts.ExtraClassifierFileName=''; % add extra for classifier file name
%% General Parameters
...
AnalysisOpts.EnforceSpockTime=nan; % are we enforcing the spock time to be a specific time?
%% General Parameters
AnalysisOpts.UseDataPointer=0; % use data pointer (matfile) instead of loading the file
...
AnalysisOpts.AreaTotalNumNeurons=[480 149 239 149 64];
%% plotting 
...
% AnalysisOpts.SampleTimInd=strcmp(AnalysisOpts.TrialTimesFields,'SAMPLE_ON');
% AnalysisOpts.FixTimInd=strcmp(AnalysisOpts.TrialTimesFields,'FIXATE_ACQUIRED');
% AnalysisOpts.RewardTimInd=strcmp(AnalysisOpts.TrialTimesFields,'CORRECT_TRIAL');
...
AnalysisOpts.SingCellAna.SkipShuffleNMdlComp=1; % are skipping the fit for shuffle and model comparision
if AnalysisOpts.SingCellAna.SkipShuffleNMdlComp
    AnalysisOpts.SingCellAna.GLMnMdlCompRuns=20; % how many runs of GLM model for model comparision
    AnalysisOpts.SingCellAna.GLMShuffleRuns=1; % how many runs of GLM for shuffling
else
    AnalysisOpts.SingCellAna.GLMnMdlCompRuns=20; % how many runs of GLM model for model comparision
...
AnalysisOpts.SingCellAna.GLM_UseSingFactorShuffwithSampling=1; % are we sampling the single factor multiple times 
AnalysisOpts.SingCellAna.GLM_UseSingFactorShuffwithSampling_N=200; % how many times are we resampling for the GLM model
...
AnalysisOpts.SingCellAna.DefaultLamdaInd=4; % what is the lambda index that we use for default
...
AnalysisOpts.SingCellAna.GLMMdlNameSet={'SensoryMotorIntegerInteractMdlToSwitch','SensoryMotorIntegerInteractMdlFromSwitch','SensoryMotorIntegerInteractMdl','SensoryMotorIntegerMdl','SensoryCatMotorRulePureInteracMdl','SensoryCatBalancedReward',...
    'SensoryMLSing','SensoryMLCatSing','SensoryMLCatObjSing','SensoryMotorInteractMdl','HybridbhvMdlFull','InferAFbhvMdlFull','SensoryCat'}; % what are the names of GLM models we are testing
AnalysisOpts.SingCellAna.GLMMdlName2Test={'SensoryMotorIntegerInteractMdl'};%{'SensoryMotorIntegerInteractMdl','SensoryMotorIntegerMdl'};%, {'SensoryCatMotorRulePureInteracMdl'};%>>>>Paper{'SensoryCat'}<<<<<; SensoryCatMotorRulePureInteracMdl
AnalysisOpts.SingCellAna.GLMMdlComp2Test={'CompareSensoryMotorIntegerInteractMdl'};%{'CompareSensoryMotorIntegerInteractMdl','CompareSensoryMotorIntegerMdl'};%,{'CompareSensoryCatMotorRulePureInteracModel'};%>>>>>Paper{'CompareSensoryCatModel'}<<<<; 
AnalysisOpts.RunThisGLMmodel=''; % which GLM model are we running right now 
...
AnalysisOpts.GLMSpockFitTime=600; %100 fit time for GLM Spock per model 
...
AnalysisOpts.PopulationAna.ProcessingStepNames={'SaveImportantData','RunSubSpace','PlotSubSpace',... %0 1 2 
    ...
    'CreatSummeryFile','ConcatinateGLMFiles','GenSummeryPlotArea','Targeted Dim Reduction(TDR)'}; %Processing Step Names % 14 15 16
...
AnalysisOpts.SubtractTrlRngNeuData4Shuff=0; % are we subtracting engaged neuron's acitivity?
AnalysisOpts.MinimizeFileName4CalShuffTrlOrder=0; % are we minimizng file name for shuffle classifier order conditions
...
AnalysisOpts.Classifier_NrepShuf=nan;% are we imposing number of resampling per rep for shuffle for classifier analysis 
AnalysisOpts.Classifier_NrepShufperFold=nan;% are we imposing number of reps for shuffle classifier analysis 
...
AnalysisOpts.GenerateCPIinfo=0;% are we generating compression index information
%% classifier tasks 
AnalysisOpts.PopulationAna.Classifier_TaskNameSet={'3D_Color_Cat_Xgen','3D_Shape_Cat_Xgen',... % Decoding Xdecoding conds
    ...
    '3D_Color_Response_XgenBalInCongV5','3D_Color_Response_XgenBalInCongV6','3D_Color_Response_XgenBalInCongV5_AreaComp',...
    'Learning3D_Shape_Color_Rule_Xgen_AltRule_RB_Step',...
    ...
    'Learning3D_Color_Response_Rule_Xgen_AltRuleRBC','Learning3D_Color_Response_Rule_Xgen_SameRuleRBC',...
    'Learning3D_Shape_Color_Color_Xgen_Angle','Learning3D_Color_Color_Response_Xgen_Angle',...
    'Learning3D_Shape_ColorBal_Color_Xgen_AngleFull','Learning3D_ShapeBal_ColorBal_Color_Xgen_AngleFull',...
    'Learning3D_Color_Color_Shape_Xgen_Angle_BlkEnd','Learning3D_Color_ColorRB_Response_Xgen_Angle_BlkEnd',...
    'Learning3D_Color_Color_Shape_Xgen_Angle_BlkEnd_Final','Learning3D_Color_ColorRB_Response_Xgen_Angle_BlkEnd_Final',...
    'Learning3D_RuleR2SeqAnalysis','Learning3D_RuleR2SeqAnalysisXgen','Learning3D_RuleR2SeqAnalysisLearning','Learning3D_RuleR2SeqAnalysisLearningV2',...
    'Learning3D_RuleR2SeqAnalysisLearningSameRule','Learning3D_RuleR2SeqAnalysisLearningAltRule','Learning3D_RuleR2SeqAnalysisLearningFinal',...
    'Learning3D_RuleR2SeqAnalysisLearningCtrl','Learning3D_RuleR2SeqAnalysisLearningCtrlMS','Learning3D_RuleR2SeqAnalysisLearningCtrlR2',...
    'Learning3D_RuleR2SeqAnalysisLearningCtrlLearning',...
    '3D_Response_Xgen_BalInCongAxisSupp','Learning3D_AxisDecoding','3D_AxisDecodingSupp','3D_Color_Shape_Axis',...
    '3D_Color_Shape_AxisV2','3D_Color_Shape_Response_BalRespDir',...
    'Learning3D_ShapeRB_ColorRB_Rule_Xgen_AltRule_RB','Learning3D_Shape_Color_Rule_Xgen_AltRuleSameRule_RB',...
    'Learning3D_Shape_Color_Rule_Xgen_SameRule_RB_EQ','Learning3D_Shape_Color_Rule_Xgen_AltRule_RBN',...
    'Learning3D_Color_Color_Color_Xgen_AltRule_RB','Learning3D_Shape_Color_Rule_Xgen_AltRule_RBUB',...
    'Learning3D_RuleR2SeqAnalysisLearningCtrlR2onR2','Learning3D_Shape_Color_Rule_Xgen_SameRule_RB_NoColDup',...
    'Learning3D_Color_Same_Alt_Rule_Stat_RB'};     % conditions for infomration evolution plots
...
AnalysisOpts.PopulationAna.Classifier_TaskConcateSpockTime=1500*ones(1,nClassifierTasks);
AnalysisOpts.Xtemp3dPlotFunctions={'PerfTripleInd','PerfTripleSuperimposed','Scores','CompareTiming','ProjScores2D'};
% AnalysisOpts.PopulationAna.Classifier_TaskNameSet={'XTemp_Rule_Xgen_BalRespDir','XTemp_Rule_Xgen_NoBal','XTemp_Rule_Xgen_NoBal_AllTrls','XTemp_Feature_Xgen','XTemp_Axis_Xgen',...
%     'XTemp_Cat_Color','XTemp_Cat_Color_InCorr','XTemp_Cat_Color_BalRespDir_Xval','XTemp_Cat_Color_BalRespDir','XTemp_Cat_Shape_BalRespDir','XTemp_Cat_Shape','XTemp_Cat_Color_Xgen_SingML',...
%     'XTemp_Cat_Color_Xgen','XTemp_Cat_Color_Xgen_BalRespDir','XTemp_Cat_Shape_Xgen_SingML','XTemp_Stim_Color_Xgen',...
%     'XTemp_Cat_Shape_Xgen','ResponseLocation','XTemp_Stim_Color','XTemp_Stim_Xgen',...
%     'ResponseLocation_Xgen_BalCorr','ResponseLocation_Xgen_BalCong',...
%     '2D_Cat_Color_Shape','2D_Cat_Color_Shape_BalRespDir','2D_Cat_Color_Resp','2D_Cat_Color_Resp_Xgen','2D_Cat_Color_Resp_Xgen_BalRespDir',...
%     '2D_Cat_Color_Color_Xgen','2D_Cat_Resp_Resp_Xgen',...
%     'Learning_Cat_Shape_Color_Xgen','Learning_Cat_Shape_Color_Xgen_AltRule','Learning_Cat_Color_Color_Xgen','Learning_Cat_Color_Color_Xgen_AltRule',...
%     'Learning_Cat_Shape_Rule_Xgen_AltRule','Learning_Cat_Shape_Rule_Xgen_AltRule_HiPerf','Learning_Cat_Color_Rule_Xgen_AltRule',...
%     'Learning_Cat_Color_Resp_Xgen_Xtemp','Learning_Cat_Color_Resp_Xgen','Learning_Cat_Shape_Resp_Xgen','Learning_Cat_Color_Resp_Xgen_AltRule','Learning_Cat_Color_Resp_Xgen_SameRule',...
%     'Learning_Cat_Shape_Resp_Xgen_AltRule','Learning_Cat_Shape_Resp_Xgen_SameRule','Learning_Cat_Color_Resp_Xgen_SMP',...
%     'Learning_Cat_Color_Resp_Xgen_AltRule150','Learning_Cat_Color_Resp_Xgen_SameRule150'};


% AnalysisOpts.PopulationAna.Classifier_TaskNameSet={     
%     %% %%%%%%%% 1D conditions
%     'XTemp_Cat_Color','XTemp_Cat_Shape',... % shape and color categorization
%     'XTemp_Cat_Color_Xgen_SingML','Cat_Color_Xgen_SingML_XTemp',...
%     'XTemp_Cat_Shape_Xgen_SingML','Cat_Shape_Xgen_SingML_XTemp',...    % cross rule shape and color categorization
%     'XTemp_Stim_Color','XTemp_Stim_Shape',...                          % shape and color stim categorizaiton
%     'XTemp_Stim_Color_Xgen','XTemp_Stim_Shape_Xgen',...                % Cross rule shape and color stim categorizaiton
%     'XTemp_Cat_Color_BalRespDir','XTemp_Cat_Shape_BalRespDir',...      % shape and color stim categorizaiton Balanced RespDir
%     'XTemp_Cat_Color_Xgen_BalRespDir','XTemp_Cat_Shape_Xgen_BalRespDir',...% Cross rule shape and color stim categorizaiton Balanced RespDir
%     'XTemp_Cat_Color_BalRespDirCong','XTemp_Cat_Color_BalRespDir_Xval',... % additional conditions
%     'ResponseLocation_BalCorr','ResponseLocation_BalCong',...            % response location dicsrimination
%     'ResponseLocation_Xgen_BalCorr','ResponseLocation_Xgen_BalCong',... % Xrule response location dicsrimination
%     'ResponseLocation_Xgen_BalCorr_XTemp','ResponseLocation_Xgen_BalCong_XTemp',... %Xrule Xtemp response location dicsrimination
%     '2D_Cat_Color_Resp_Xgen',...                                         % 2D color and response categorization
%     'Learning_Cat_Color_Color_Xgen_AltRule','Learning_Cat_Shape_Color_Xgen_AltRule',... % sensory information durding learning Alternate rule seq 
%     'Learning_Cat_Shape_Color_Xgen_SameRule',...% sensory information durding learning same rule seq
%     'Learning_Cat_Shape_Rule_Xgen_AltRule','Learning_Cat_Shape_Rule_Xgen_AltRule_HiPerf',...% rule information durding learning alt rule seq high/low perf
%     'Learning_Cat_Color_Rule_Xgen_AltRule','Learning_Cat_Color_Rule_Xgen_AltRule_HiPerf',...% rule information durding learning alt rule seq high/low perf    
%     'Learning_Cat_Color_Rule_Xgen_AltRule_Test',... % test conditions
%     'FineTune_Cat_Color_Xgen','FineTune_Stim_Color','FineTune_Stim_Color_Kernel','Learning_Cat_Color_Resp_Xgen',...% classifier fine tune conditions  
%     'XTemp_AllObjects','XTemp_Abstract_Color_Xgen_SingML',...
%     'Learning_Congruency_Rule_Xgen_AltRule','Learning_Congruency_Shape_Color_Xgen_AltRule','Learning_Congruency_Shape_Color_Xgen_AltRule_Test',...% congruency conditions
%     'Learning3D_Color_Shape_Rule_Xgen_AltRule','Learning3D_Color_Shape_Rule_Xgen_AltRule_HiTh','Learning3D_Color_Shape_Rule_Xgen_SameRule',... % 3D conditions
%     'Learning3D_Color_Shape_Response_Xgen','Learning3D_Color_Shape_Shape_Xgen','Learning3D_Color_Color_Rule_Xgen_AltRuleR1',...
%     '3D_Color_Cat_Xgen'};
% AnalysisOpts.PopulationAna.Classifier_TaskSpockTime=[BT BT BT 2000 BT 2000 BT*ones(1,14) 2000 2000 ,... % up to learning
%     5*BT*ones(1,9) ,... % up to fine tune
%     1500 BT 5*BT 5*BT 5*BT 5*BT 5*BT 5*BT 60 2*BT 2*BT BT BT BT BT BT];

AnalysisOpts.PopulationAna.Classifier_ComparisionNameSet={'3D_Cat_Color_Area_Summery','3D_Cat_Color_Response_Compare','3D_Cat_Color_XgenCol_Compare','3D_Response_XgenResponse_Compare','3D_Xgen_Compare',...
...
AnalysisOpts.PaperFigs.Decoding.XTicksLearning=[40 60 80 100];% [60 80 100 120]
...
AnalysisOpts.ntrlPerCondTestArea=nan; % number of training trails per area
...
AnalysisOpts.NeuResamplingSubPopSize=50;  % Size of the subpopulation we are resampling
AnalysisOpts.MaxTrlTestLearningArea=125; % what is the maximum number of trials for test in learning 
%AnalysisOpts.AxisSuppAnalysisAreaYlim={[2 11],[0 10],[1 23],[1 23],[0 36]};
%AnalysisOpts.AxisSuppAnalysisAreaYlim={[2 15],[],[1 23],[1 23],[]};
AnalysisOpts.AxisSuppAnalysisAreaYlim={[],[],[],[],[]};

AnalysisOpts.ClassifierLearningParamsFields={'ntrlPerCondTestArea','ntrlPerCondArea','NTrlRngTrainLearningArea',...
    'NTrlRngTestLearningArea','LimitFromSwitchPerfArea',...
    'CalShuffTrlOrderClassifier','GetOnlyShuffLabelsClassifier','MaxTrlTestLearningArea',...
    'pvalClassifierAnalysis_ClusterCorrect','NTrlStpLearningArea','NeuResamplingSubPopSize',...
    'Classifier_TrlShuff_ShuffMethod','Classifier_TrlShuff_TrialSet','Classifier_TrlShuff_GenClassifierSpecs'};

%AnalysisOpts.ClassifierLearningParamsFieldsStr=[@(y) [cellfun(@(x) sprintf('''%s'',%i,',x,AnalysisOpts.(x)),AnalysisOpts.ClassifierLearningParamsFields,'UniformOutput',0)]];
% AnalysisOpts.ClassifierLearningParamsFieldsStr={'ntrlPerCondArea',AnalysisOpts.ntrlPerCondArea,'NTrlRngTrainLearningArea',AnalysisOpts.NTrlRngTrainLearningArea,...
%     'NTrlRngTestLearningArea',AnalysisOpts.NTrlRngTestLearningArea,...
%     'LimitFromSwitchPerfArea',AnalysisOpts.LimitFromSwitchPerfArea,'Classifier_FileDateTimeTh',char(AnalysisOpts.Classifier_FileDateTimeTh)};
% AnalysisOpts.ClassifierLearningParamsFieldsSpf=[repmat({"'%s'",'%i'},1,length(AnalysisOpts.ClassifierLearningParamsFields)) {"'%s'","'%s'"}];
%FprintfFields=arrayfun(@(x) fprintf(["'%s'",'%i']))
...
AnalysisOpts.factornames={'ColorML','ShapeML','ColorCat','ShapeCat','Rule','ResponseLoc','Reward','RT','Time',...
                ...
                'BhvPerfShape50','BhvPerfColor50','BhvPerfShape10','BhvPerfColor10','BlkSeqBack','BlkSeqForw','BlkSeqCust','QuadrantObjs'};
AnalysisOpts.FactorInds2Keep={'ColorML','ShapeML','Reward','ResponseLoc','RuleInfo',...
    'BhvPerfShape50','BhvPerfColor50','BhvPerfShape10','BhvPerfColor10'}; % factors we are keeping for further analysis 
AnalysisOpts.Nfactornames=length(AnalysisOpts.factornames);
...
AnalysisOpts.Classifier_TrlShuff_TrendCorrMethod='TrlOrderShuffle';%'TrlOderShuffle: shuffle the order of the trials; regression or Kendall(corretion) Modified_MannKendall
...
AnalysisOpts.Modified_MannKendall_significance_value_ac = 0.001; %predetermined level for selecting only those autocorrelation lag values that are statistically significant. Two-tailed test
...
AnalysisOpts.Classifier_TrlShuff_SaveObservedClassifierSpecs=0; % are we saving observed classifier specs
AnalysisOpts.Classifier_TrlShuff_LoadObservedClassifierSpecs=1; % are we loading the observed classifier specs
AnalysisOpts.Classifier_TrlShuff_ShuffMethod=''; % could be "RND" or "FEA" How are are we shuffling is it based on feature or we are suffling all
AnalysisOpts.Classifier_TrlShuff_TrialSet=''; % "Could be "FL" which is "First and Last" or "All"
AnalysisOpts.Classifier_TrlShuff_GenClassifierSpecs=0; %Are we generating classifier specs
AnalysisOpts.TrendTestMethod='ClusterCorrectionTrlorderShuff'; %Could be ClusterCorrectionTrlorderShuff where we shuffle trial order and use cluster correciuton,Modified_MannKendall, or SamplingShuffle(we shuffle the samples) or (Permutation) we shuffle the accuracy 
AnalysisOpts.SkipLoadingShuffleFile=0;% are we skiping to load the shuffle file?
AnalysisOpts.ClassifierNameExtra=''; %Extra text added to the classifier name 
AnalysisOpts.CalShuffTrlOrderClassifier_RunAllTrlRng=0; % are we doing the trial shuffle for all of the trial blocks
AnalysisOpts.EqualizeTrialsXConds=1; % are we equalizing conditions across trials 
AnalysisOpts.Subtract4TrialRange=0; %  % are we subtracting the mean rule data for trials of each trial period
AnalysisOpts.SkipBalancing4TrlOrderShuffle=0; % are we skipping balancing for trial order shuffle?
AnalysisOpts.UsePreviousShufTxtNotation=0; % if we want to use subscript _Shuf instead of _ShLb for shufflelabels
AnalysisOpts.IncludeAllReps4ShuffFold=0;% Are we including data for all reps for shuffle fold in the concatinated data
AnalysisOpts.LimitNumReps4ShuffFold=[];% AnalysisOpts.IncludeAllReps4ShuffFold has to be true, are we limiting the number of repetitions included in the concatinated data e.g. only the first 20 reps
AnalysisOpts.CalShuffTrlOrderClassifier_StatCompMethod='TFPW_Y_p'; % what is the method where we compute the statistical test for shuffle trial order test
AnalysisOpts.CalShuffTrlOrderClassifier_ClusterTH=30;%24; % we are using TJB code for luster correction and this is the threshold in percent 
...
%% Targeted Dim Reduction Analysis(TDR)
AnalysisOpts.TDRSpockTime=500; % time to run TDR on spock
...
%AnalysisOpts.MorphlevelsShpRGB=AnalysisOpts.MorphlevelsShpRGBInc50([1 2 4:6 8],:);
AnalysisOpts.MorphlevelsShpRGB=HEX2RGB({'#2f4858', '#33658a', '#f6ae2d' ,'#f26419', '#beb583', '#86bbd8'}); %https://coolors.co/2f4858-33658a-86bbd8-beb583-f6ae2d-f26419
AnalysisOpts.ProtoypeImgRGB=[245, 66, 66; 78, 245, 66;66, 96, 245; 239, 66, 245];

%% define my color paletts 
AnalysisOpts.ColorPalett1=HEX2RGB({'#264653', '#2a9d8f', '#8ab17d', '#e9c46a', '#f4a261', '#e76f51'}); % charcoal persian green olivine saffron sandy brown burnt sienna https://coolors.co/264653-2a9d8f-8ab17d-e9c46a-f4a261-e76f51
...
AnalysisOpts.AxisColors=AnalysisOpts.ColorPalettCamden(1:2,:);
AnalysisOpts.OffAxisxRule3Colors=AnalysisOpts.ColorPalettCamden(1:2,:);
AnalysisOpts.OffAxisxRule1Colors=AnalysisOpts.ColorPalettCamden(1:2,:);
AnalysisOpts.OffAxisxRule2Colors=AnalysisOpts.ColorPalettCamden(1:2,:);
AnalysisOpts.AxisxRule3Colors=AnalysisOpts.ColorPalettCamden(1:2,:);
AnalysisOpts.AxisxRule1Colors=AnalysisOpts.ColorPalettCamden(1:2,:);
AnalysisOpts.AxisxRule2Colors=AnalysisOpts.ColorPalettCamden(1:2,:);
...
AnalysisOpts.PaperSpec.AxisSupp_Start=-0.6;
AnalysisOpts.PaperSpec.AxisSupp_End=0.6;
