...
function [ClassifierOpts]=CreateShuffleDist4ClassifierCorrectShuffV4(obj,FactorData,ClassifierOpts,FactorLevelComb,DimNum,Cond,TrlRng)
global AnalysisOpts
...
if obj.CalShuffTrlOrder % if we are shuffleing the trial order then we will need to do it within the target factor and all of the balances we are doing
    ...
    [ShuffLbl,TrialIndex,TrainCondIndex,BlkOrderShuff,RevTrlNumShuff]=SamplefromFactor(ManData,ClassifierOpts,'TrialNum',TargetFields,FactorData,DimNum,IndTestCond,SeqHistVal,Cond,TrlTypeVal_Test);
else
    TrainCond=TrainCond{Cond};
    IndTrainCond=cell2mat(arrayfun(@(x) find(FactorLevelComb(:,2)==x)',TrainCond,'UniformOutput',0));
    [ShuffLbl,TrialIndex,TrainCondIndex,BlkOrderShuff,RevTrlNumShuff]=SamplefromFactor(ManData,ClassifierOpts,TargetFactor,TargetFields,FactorData,DimNum,IndTrainCond,SeqHistVal,Cond,TrlTypeVal_Train);
end
...
function [ShuffLbl,TrialIndex,TrainCondIndex,BlkOrderShuff,RevTrlNumShuff]=SamplefromFactor(ManData,ClassifierOpts,TargetFactor,TargetFields,FactorData,DimNum,IndTrainCond,SeqHistVal,Cond,TrlTypeVal)
