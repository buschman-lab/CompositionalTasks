...
function [ClassifierOpts]=CreateShuffleDist4ClassifierCorrectShuffV2(obj,FactorData,ClassifierOpts,FactorLevelComb,DimNum,Cond)
global AnalysisOpts
...
if contains(TargetFactor,'Color') |  contains(TargetFactor,'Shape')
    ...
    if sum(contains(TargetFields,'Congruency','IgnoreCase',true))
        [ShuffLbl,TrialIndex,TrainCondIndex]=SamplefromFactor(ManData,ClassifierOpts,TargetFactor,AlternateFactor,FactorData,DimNum,IndTrainCond,SeqHistVal);
    elseif sum(contains(TargetFields,'ResponseLoc','IgnoreCase',true))
        [ShuffLbl,TrialIndex,TrainCondIndex]=SamplefromFactor(ManData,ClassifierOpts,TargetFactor,'ResponseLoc',FactorData,DimNum,IndTrainCond,SeqHistVal);
    elseif sum(contains(TargetFields,'Reward','IgnoreCase',true))
        [ShuffLbl,TrialIndex,TrainCondIndex]=SamplefromFactor(ManData,ClassifierOpts,TargetFactor,'Reward',FactorData,DimNum,IndTrainCond,SeqHistVal);
    end

elseif sum(contains(TargetFactor,'ResponseLoc','IgnoreCase',true)) | sum(contains(TargetFactor,'Rule','IgnoreCase',true)) % we don't balance for anything


  AlternateFactor=TargetFields{~contains(TargetFields,'SeqHist','IgnoreCase',true)};
  [ShuffLbl,TrialIndex,TrainCondIndex]=SamplefromFactor(ManData,ClassifierOpts,TargetFactor,AlternateFactor,FactorData,DimNum,IndTrainCond,SeqHistVal);
    
%elseif sum(contains(TargetFactor,'Rule','IgnoreCase',true)) % we don't balance for anything
   
%    [ShuffLbl,TrialIndex,TrainCondIndex]=SamplefromFactor(ManData,ClassifierOpts,TargetFactor,'',FactorData,DimNum,IndTrainCond,SeqHistVal);
...
function [ShuffLbl,TrialIndex,TrainCondIndex]=SamplefromFactor(ManData,ClassifierOpts,TargetFactor,AlternateFactor,FactorData,DimNum,IndTrainCond,SeqHistVal)
...
for NN=1:NNeu %loop on neurons
    ...
    if isempty(AlternateFactor)
        ...
        if ~isnan(SeqHistVal)
            for ss=1:ClassifierOpts.NrepShufperFold
                Ncat1=sum(AltFactor==UniqAltFactorVals(1) & SeqhistValThisNeu==SeqHistVal);
                Ncat2=sum(AltFactor==UniqAltFactorVals(2) & SeqhistValThisNeu==SeqHistVal);

                LabelInd1=randsample(find(AltFactor==UniqAltFactorVals(1) & SeqhistValThisNeu==SeqHistVal),Ncat1);
                LabelInd2=randsample(find(AltFactor==UniqAltFactorVals(2) & SeqhistValThisNeu==SeqHistVal),Ncat2);
                % keep the labels that are not affected 
                 ShuffLbl{NN}(:,ss)=AllLabels;
                % change the labels thaat are affected
                ShuffLbl{NN}(AltFactor==UniqAltFactorVals(1) & SeqhistValThisNeu==SeqHistVal,ss)=AllLabels(LabelInd1);
                ShuffLbl{NN}(AltFactor==UniqAltFactorVals(2) & SeqhistValThisNeu==SeqHistVal,ss)=AllLabels(LabelInd2);
            end
        else
            for ss=1:ClassifierOpts.NrepShufperFold
