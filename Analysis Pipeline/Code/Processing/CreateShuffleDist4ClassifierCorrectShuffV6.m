...
function [ClassifierOpts,NewFactorDataSubTract]=CreateShuffleDist4ClassifierCorrectShuffV6(obj,FactorData,ClassifierOpts,FactorLevelComb,DimNum,Cond,TrlRng)
global AnalysisOpts
...
if ClassifierOpts.ShuffleSeqHist;ShuffleSeqHist=1;end % if we are shuffling the seqhist
...
if ShuffleSeqHist % remove seqhist from target fields
    TargetFields=TargetFields(~strcmp(TargetFields,'SeqHist'));
end
...
if obj.CalShuffTrlOrder | ShuffleSeqHist
...
if obj.CalShuffTrlOrder | ShuffleSeqHist % if we are shuffleing the trial order then we will need to do it within the target factor and all of the balances we are doing
    ...
    if ShuffleSeqHist
              [ShuffLbl,TrialIndex,TrainCondIndex,BlkOrderShuff,RevTrlNumShuff,EngagedTrails,AvgSpkData,EngageTrlsTrlNum,NewFactorDataSubTract]=SamplefromFactor(ManData,ClassifierOpts,...
            'SeqHist',TargetFields,FactorData,DimNum,IndTestCond,SeqHistVal,Cond,TrlTypeVal_Test,TrialRngInc,ShuffleTrlSet,TestTrlRange);
    else
        [ShuffLbl,TrialIndex,TrainCondIndex,BlkOrderShuff,RevTrlNumShuff,EngagedTrails,AvgSpkData,EngageTrlsTrlNum,NewFactorDataSubTract]=SamplefromFactor(ManData,ClassifierOpts,...
            'TrialNum',TargetFields,FactorData,DimNum,IndTestCond,SeqHistVal,Cond,TrlTypeVal_Test,TrialRngInc,ShuffleTrlSet,TestTrlRange);
    end
else
    ...
    [ShuffLbl,TrialIndex,TrainCondIndex,BlkOrderShuff,RevTrlNumShuff,EngagedTrails,AvgSpkData,EngageTrlsTrlNum,NewFactorDataSubTract]=SamplefromFactor(ManData,ClassifierOpts,...
        TargetFactor,TargetFields,FactorData,DimNum,IndTrainCond,SeqHistVal,Cond,TrlTypeVal_Train,TrialRngInc,ShuffleTrlSet,TrainTrlRange);
end
...
for NeuG=1:length(NeuGroups) % loop through the groups and copy the shuffle for each one
    ...
    if length(ThisNe)>1
        ...
        EngagedTrails(ThisNe(2:end))=EngagedTrails(ThisNe(1));
    end
end
...
ClassifierOpts.ClassifierShuffleEngagedTrails{DimNum}=EngagedTrails;
ClassifierOpts.NeuGroups=NeuGroups;
ClassifierOpts.ClassifierShuffleAvgSpkData{DimNum}=AvgSpkData; % average spiking data for the conditions of intrest
ClassifierOpts.ClassifierShuffleEngageTrlsTrlNum{DimNum}=EngageTrlsTrlNum; % what are the trials that are engaged for this trial number
end

function [ShuffLbl,TrialIndex,TrainCondIndex,BlkOrderShuff,RevTrlNumShuff,EngagedTrails,AvgSpkData,EngageTrlsTrlNum,NewFactorDataSubTract]=SamplefromFactor(ManData,ClassifierOpts,...
    TargetFactor,TargetFields,FactorData,DimNum,IndTrainCond,SeqHistVal,Cond,TrlTypeVal,TrialRngInc,ShuffleTrlSet,TrialRange)
...
NewFactorDataSubTract=FactorData;
for NN=1:NNeu %loop on neurons
    ...
    % if we have an extra factor we are limiting the data for limit the trials based on that as well
    if isfield(ClassifierOpts,'LimitTrainTrialsBasedonFactor') | isfield(ClassifierOpts,'LimitTestTrialsBasedonFactor')
        if strcmp(ShuffleTrlSet,'Train')
            IncludedTrials=LimitTrialsBasedonFactor(FactorDataNeu,ClassifierOpts.LimitTrainTrialsBasedonFactor{Cond},...
                ClassifierOpts.LimitTrainTrialsBasedonFactor_Val{Cond},ClassifierOpts.LimitTrainTrialsBasedonFactor_Operation{Cond});
        elseif strcmp(ShuffleTrlSet,'Test')
            IncludedTrials=LimitTrialsBasedonFactor(FactorDataNeu,ClassifierOpts.LimitTestTrialsBasedonFactor{Cond},...
                ClassifierOpts.LimitTestTrialsBasedonFactor_Val{Cond},ClassifierOpts.LimitTestTrialsBasedonFactor_Operation{Cond});
        end
%       else
%          IncludedTrials=[];
    end
    ...
    ThisSpkData=ManData.ReshapeCell2Mat(FactorDataNeu.data(IndTrainCond),62);
    ...
    % if we have the axis condition with response location as the field for congruency
    if strcmp(TargetFactor,'Axis') & strcmp(TargetFields{1},'ResponseLoc') & length(TargetFields)==1% merge 1,3 and 2,4
        AltFactor{1}(AltFactor{1}==3)=1;
        AltFactor{1}(AltFactor{1}==4)=2;
        UniqAltFactorVals=unique(AltFactor{1});
    end

    % save which are the trials we are taking
    EngagedTrails{NN}=SeqhistValThisNeu==ThisSeqHistVal & TrlTypeValThisNeu==ThisTrlTypeVal & ThisIncTrlsLimitFactor;

        % now take average of neural data for trials that are engage and are in each of the trialrange cells
        if AnalysisOpts.SubtractTrlRngNeuData4Shuff
            for iii=1:length(TrialRange)
                TrainTrlNumInc{iii}=arrayfun(@(x) logical(sum(TrialRange{iii}==x)),ThisTrainTrlNum);
                AvgSpkData{NN}{iii}=mean(ThisSpkData(EngagedTrails{NN} & TrainTrlNumInc{iii},:));
                EngageTrlsTrlNum{NN}{iii}=EngagedTrails{NN} & TrainTrlNumInc{iii};
                if ClassifierOpts.Subtract4TrialRange
                    for iiiCond=unique(TrainCondIndex{NN})
                        NewFactorDataSubTract(NN).data{iiiCond}(EngageTrlsTrlNum{NN}{iii}(TrainCondIndex{NN}==iiiCond),:)=NewFactorDataSubTract(NN).data{iiiCond}(EngageTrlsTrlNum{NN}{iii}(TrainCondIndex{NN}==iiiCond),:)- AvgSpkData{NN}{iii};
                    end
                end
            end
        else
    ...
    if isempty(TargetFields) | ClassifierOpts.SkipShuffleBalancing(DimNum) | ClassifierOpts.SkipBalancing4TrlOrderShuffle% if we don't have any balancing condition for shuffling
        ...
        elseif nTargetFields==3
            ...
            if strcmp(TargetFactor,'TrialNum')
                tt=1;
                for trl=1:length(TrialRange)
                    for ss=1:ClassifierOpts.NrepShufperFold
                        for ll=1:size(UniqAltFactorVals,1)
                            ThisTrials=AllLabels>=TrialRange{trl}(1) & AllLabels<=TrialRange{trl}(end) & EngagedTrails{NN};
                            AltFactorCheck=arrayfun(@(x) ThisTrainFactors(ThisTrials,x),TargetFieldsInd,'UniformOutput',0);
                            ...
                            ThisTrialsShuff=ShuffLbl{NN}>=TrialRange{trl}(1) & ShuffLbl{NN}<=TrialRange{trl}(end) & EngagedTrails{NN};
                            AltFactorShuffCheck=arrayfun(@(x) ThisTrainFactors(ThisTrialsShuff,x),TargetFieldsInd,'UniformOutput',0);
                    ...
                    NcatCheckShuffAll(tt,:)=NcatCheckShuff;NcatCheckAll(tt,:)=NcatCheck;
                    tt=tt+1;
                end
                if sum(sum(NcatCheckAll==NcatCheckShuffAll))~=size(UniqAltFactorVals,1)*length(TrialRange);error('in the sum of trials');end
            end
        else
            for ss=1:ClassifierOpts.NrepShufperFold
                for ll=1:size(UniqAltFactorVals,1)
...
switch FactorOperation
    ...
    case 'smaller'
        IncludedTrls=cellfun(@(x) cell2mat(arrayfun(@(y) (x(:,FactorInd)<=y)',FactorValues,'Uniformoutput',0)),FactorData.AllFactors,'uniformoutput',0);
    case 'interval'
        IncludedTrls=cellfun(@(x) cell2mat(arrayfun(@(y) (x(:,FactorInd)>=FactorValues(y,1) & x(:,FactorInd)<=FactorValues(y,2))',1:size(FactorValues,1),'Uniformoutput',0)),FactorData.AllFactors,'uniformoutput',0);
end
